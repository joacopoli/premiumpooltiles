    ...
    <section id="projects" className="py-20 sm:py-32 bg-white scroll-mt-24">
      <div className="max-w-7xl mx-auto px-6 sm:px-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24 items-center">
          <div>
            <span className="text-rose-700 font-bold uppercase tracking-[0.4em] text-[10px] mb-4 block">Our Methodology</span>
          ...
          <div className="space-y-10">
            {steps.map((step, i) => (
              <div key={i} className="flex gap-6 items-start group">
                <span className="text-2xl font-bold text-rose-200 group-hover:text-rose-700 transition-colors duration-500">0{i + 1}</span>
                <div>
        ...
        <div className="mb-12 flex flex-col sm:flex-row justify-between items-end gap-6">
          <div className="max-w-xl">
            <span className="text-rose-700 font-bold uppercase tracking-[0.4em] text-[10px] mb-4 block">Signature Gallery</span>
            <h2 className="text-3xl font-bold text-slate-900 uppercase tracking-tight">Recent Masterpieces</h2>
          </div>
          <button className="text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-rose-700 transition-colors border-b border-slate-200 pb-1">
            View Full Portfolio
          </button>
        </div>
          ...
          {projects.map((project, idx) => (
            <div key={idx} className="group relative overflow-hidden rounded-sm shadow-sm aspect-[16/10]">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-8 sm:p-12">
                <p className="text-rose-700 text-[10px] font-bold uppercase tracking-[0.3em] mb-2 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">{project.location}</p>
