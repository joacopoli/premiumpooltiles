    ...
    <div className="pt-32 pb-24 bg-white">
      <div className="max-w-3xl mx-auto px-6">
        <div className="text-center mb-16">
          <span className="text-rose-700 font-bold uppercase tracking-[0.4em] text-[10px] mb-4 block">Knowledge Base</span>
        ...
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            <div key={i} className="border border-slate-100 rounded-sm overflow-hidden">
              ...
              >
                <span className={`font-bold text-xs uppercase tracking-widest transition-colors ${openIndex === i ? 'text-rose-700' : 'text-slate-700'}`}>
                  {faq.q}
                </span>
                <span className="text-rose-700 font-bold text-xl">{openIndex === i ? '−' : '+'}</span>
              </button>
              {openIndex === i && (
        ...
        <div className="mt-16 p-8 bg-rose-50 text-center rounded-sm">
          <p className="text-slate-900 font-bold text-xs uppercase tracking-widest mb-4">Still have questions?</p>
          <a href="https://wa.me/5491155867074" className="inline-block bg-slate-950 text-white px-8 py-4 rounded-sm font-bold uppercase tracking-widest text-[10px] hover:bg-rose-800 transition-all">
            Message an Expert
          </a>
        </div>
      </div>
